...
class _VentaDetalleState extends State<VentaDetalle> {
      ...
      builder: (_) => StatefulBuilder(builder: (ctx, setS) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextField(controller: montoCtrl,
              ...
              decoration: const InputDecoration(prefixText: '\$ ', prefixStyle: TextStyle(color: AppTheme.primary, fontSize: 18, fontWeight: FontWeight.w700))),
          const SizedBox(height: 16),
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 36, height: 4, decoration: BoxDecoration(color: AppTheme.border, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 16),
          Text('Cobrar a ${cobro['nombre_cliente']}', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 17, fontWeight: FontWeight.w700)),
          ...
          TextField(
            ...
            decoration: const InputDecoration(prefixText: '\$ ', prefixStyle: TextStyle(color: AppTheme.success, fontSize: 26, fontWeight: FontWeight.w800)),
          ),
          const SizedBox(height: 24),
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: () async {
            ...
            style: ElevatedButton.styleFrom(backgroundColor: AppTheme.success, foregroundColor: AppTheme.background),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Text('RENTABILIDAD', style: TextStyle(color: AppTheme.textMuted, fontSize: 11, letterSpacing: 1.2, fontWeight: FontWeight.w600)),
                  const Spacer(),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(
                      color: ganancia >= 0 ? AppTheme.success.withOpacity(0.1) : AppTheme.danger.withOpacity(0.1),
                      borderRadius: BorderRadius.circular(5),
                    ),
                    child: Text(
                      ganancia >= 0 ? '↑ Ganancia' : '↓ Pérdida',
                      style: TextStyle(color: ganancia >= 0 ? AppTheme.success : AppTheme.danger, fontSize: 11, fontWeight: FontWeight.w700),
                    ),
                  ),
                ]),
                const SizedBox(height: 20),
                ...
                if (esperado > cobrado) ...[
                  const SizedBox(height: 10),
                  _barraComparativa('Esperado', esperado, max(max(invertido, esperado), 0.01), AppTheme.primary, dashed: true),
                ],
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(context, true),
              style: ElevatedButton.styleFrom(backgroundColor: AppTheme.danger), child: const Text('Eliminar')),
        ],
      ),
    );
    if (ok != true) return;
  ...
  Widget _barraComparativa(String label, double valor, double max, Color color, {bool dashed = false}) {
    final pct = max > 0 ? (valor / max).clamp(0.0, 1.0) : 0.0;
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(label, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12)),
        Text('\$${valor.toStringAsFixed(2)}', style: TextStyle(color: color, fontWeight: FontWeight.w700, fontSize: 13)),
      ]),
      const SizedBox(height: 6),
      Stack(children: [
        Container(height: 10, decoration: BoxDecoration(color: AppTheme.surfaceAlt, borderRadius: BorderRadius.circular(5))),
    ...
    child: GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: selected == value ? AppTheme.primary : AppTheme.border, width: selected == value ? 1.5 : 1),
        ),
...
class _ClienteTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final montoCobrado = cobro['monto_cobrado'] != null ? double.tryParse(cobro['monto_cobrado'].toString()) : null;
    final condicion = cobro['condicion_pago'] == 'plazo' ? 'A plazo · ${cobro['dias_plazo']}d' : 'Contra entrega';
    ...
    return Container(
      ...
      child: Row(children: [
        // Avatar cliente
        CircleAvatar(
          backgroundColor: cobrado ? AppTheme.success.withOpacity(0.15) : AppTheme.primary.withOpacity(0.1),
          radius: 20,
          child: Text(
            (cobro['nombre_cliente'] as String? ?? '?').isNotEmpty
                ? (cobro['nombre_cliente'] as String)[0].toUpperCase() : '?',
            style: TextStyle(color: cobrado ? AppTheme.success : AppTheme.primary, fontWeight: FontWeight.w800, fontSize: 16),
          ),
        ),
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(cobro['nombre_cliente'] ?? '', style: TextStyle(
            ...
            decoration: cobrado ? TextDecoration.none : null,
          )),
          const SizedBox(height: 3),
          Row(children: [
            Container(
            ...
            Row(children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: AppTheme.success.withOpacity(0.12), borderRadius: BorderRadius.circular(5),
                    border: Border.all(color: AppTheme.success.withOpacity(0.3)),
                  ),
                  child: const Text('Cobrar', style: TextStyle(color: AppTheme.success, fontSize: 11, fontWeight: FontWeight.w700)),
                ),
              ),
            ])
          else
...
// Widget de separador con label (importado del theme si ya existe)
