...
async function generarMovimientosPeriodo(presupuestoId, periodoId, firebaseUid) {
  const [gastos] = await db.execute(
    `SELECT * FROM gastos WHERE presupuesto_id = ? AND firebase_uid = ?
     AND (tipo = 'fijo' OR (tipo = 'fijo_x_periodo' AND numero_quincena > 0))`,
    [presupuestoId, firebaseUid]
  );
...
app.post('/presupuestos', async (req, res) => {
  ...
  try {
    ...
    await crearPrimerPeriodo(presupuestoId, firebase_uid, tipo_periodo, dia_inicio_periodo);
    res.status(201).json({ id: presupuestoId, nombre, monto_total, tipo_periodo, dia_inicio_periodo });
  } catch (error) {
...
app.get('/presupuestos', async (req, res) => {
  ...
  try {
    const [results] = await db.execute(
      `SELECT * FROM presupuestos WHERE firebase_uid = ? ORDER BY id DESC`,
      [firebase_uid]
    );
    res.json(results);
  } catch (error) {
...
app.post('/gastos', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id: result.insertId, presupuesto_id, descripcion, monto, tipo, fecha, pagado: 0, firebase_uid });
  } catch (error) {
    console.error(error);
...
app.put('/presupuestos/:id/gastos/reanudar-fijos', async (req, res) => {
  ...
  try {
    const [result] = await db.execute(
      `UPDATE gastos SET pagado = 0 WHERE presupuesto_id = ? AND tipo = 'fijo'`,
      [id]
    );
  ...
  try {
    ...
    const [gastos] = await db.execute(
      `SELECT g.* FROM gastos g
       WHERE g.presupuesto_id = ? AND g.firebase_uid = ?
         AND (g.tipo = 'no fijo' OR (g.tipo = 'fijo_x_periodo' AND (g.numero_quincena IS NULL OR g.numero_quincena > 0)))
...
app.put('/movimientos/:id/pagar', async (req, res) => {
  ...
  try {
    ...
    await db.execute(
      `UPDATE movimientos
       SET pagado = ?, monto_pagado_real = ?, pagado_por_uid = ?,
